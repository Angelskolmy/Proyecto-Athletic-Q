...
class Historial_usuario(models.Model):
    ...
    def _str_(self):
        return (
            f"Id_historial{self.Id_historial} - Fecha_y_hora{self.Fecha_y_hora} - "
            f"id_usuario{self.id_usuario} - TIpo_Movimiento{self.TIpo_Movimiento} - "
            f"Modulo{self.Modulo} - Nombre_Objeto{self.Nombre_Objeto} - Id_Objeto{self.Id_Objeto}"
        )