...
function capturarYEnviarValidacion() {
    const template = localStorage.getItem("intermediate");

    if (!template) {
        alert("Captura la huella primero");
        return;
    }

    document.getElementById("huella_lectura").value = template;
...
$("#save").on("click",function(){
    if(localStorage.getItem("imageSrc") == "" || localStorage.getItem("imageSrc") == null || document.getElementById('imagediv').innerHTML == ""){
        ...
        var vDiv = document.getElementBytId('imageGallery');
