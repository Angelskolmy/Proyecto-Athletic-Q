...
def listEst_Sal(request, Id_producto): 
    ...
    return render (request,'templates_productos/Entrada_salida.html', Aquelarre) 


def IngresarEntradaOSalida(request, Id_producto):  
    ...
    return render(request, 'templates_productos/ingresar_Entrada_salida.html', cifrer) 

def BUscadorE (request): 
    ...
    if Fecha: 
        ...
        sqlBUsq44= 'SELECT producto.*, Salidas_Entradas.* FROM Salidas_Entradas join producto on  Salidas_Entradas.Id_ProAsoc = producto.Id_producto WHERE Salidas_Entradas.Fecha_cambio= %s and Salidas_Entradas.Id_ProAsoc= %s' 

        busqRaw= Salidas_Entradas.objects.raw(sqlBUsq44,[Fecha,Id_producto]) 

    if Cambio_precio: 

        sqlBUsq44= 'SELECT producto.*, Salidas_Entradas.* FROM Salidas_Entradas join producto on  Salidas_Entradas.Id_ProAsoc = producto.Id_producto WHERE Salidas_Entradas.Cambio_precio= %s and Salidas_Entradas.Id_ProAsoc= %s'  

        busqRaw= Salidas_Entradas.objects.raw(sqlBUsq44,[Cambio_precio,Id_producto])  

    if Fecha and Cambio_precio: 

        sqlBUsq44= 'SELECT producto.*, Salidas_Entradas.* FROM Salidas_Entradas join producto on  Salidas_Entradas.Id_ProAsoc = producto.Id_producto WHERE Salidas_Entradas.Fecha_cambio= %s and Salidas_Entradas.Cambio_precio= %s and Salidas_Entradas.Id_ProAsoc= %s' 
    ...
    return render(request,'templates_productos/Entrada_salida.html',Aquelarre2)
